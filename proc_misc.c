...
int rtk_stack_check(void)
{
	unsigned int i,s;

	s = (unsigned int)init_stack;

	for (i=(s+sizeof(struct task_struct)); i<(s+KERNEL_STACK_SIZE); i+=4) {
		if (READ_MEM32(i) != 0)
			break;
	}
	return (i - (s+sizeof(struct task_struct)));
}

static int rtk_stack_read_proc(char *page, char **start, off_t off,
				 int count, int *eof, void *data)
{
	int len;

	len = sprintf(page, "total stack size: %d bytes, remained minimum stack size ever: %d bytes.\n", 
